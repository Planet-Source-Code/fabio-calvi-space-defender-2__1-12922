...
If BadGuys(X).activated = 0 Then GoTo 10
    Set BadGuys(X).PicT = Form2.Alien7(BadGuys(X).indice)
    Set BadGuys(X).mask = Form2.Alien7M(BadGuys(X).indice)
    BadGuys(X).xsize = Form2.Alien7(BadGuys(X).indice).ScaleWidth
    BadGuys(X).ysize = Form2.Alien7(BadGuys(X).indice).ScaleHeight
   ...
   If BadGuys(X).indice > 4 Then BadGuys(X).indice = 0
