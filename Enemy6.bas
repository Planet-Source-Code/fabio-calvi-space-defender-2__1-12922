...
If BadGuys(X).activated = 0 Then GoTo 10
    Set BadGuys(X).PicT = Form2.Alien4(BadGuys(X).indice)
    Set BadGuys(X).mask = Form2.Alien4M(BadGuys(X).indice)
    BadGuys(X).bulletlxpos = BadGuys(X).xsize / 2
    BadGuys(X).bulletlypos = BadGuys(X).ysize / 2
    BadGuys(X).xsize = Form2.Alien4(BadGuys(X).indice).ScaleWidth
    BadGuys(X).ysize = Form2.Alien4(BadGuys(X).indice).ScaleHeight
   ...
   If BadGuys(X).indice > 12 Then BadGuys(X).indice = 0
