...
If BadGuys(X).activated = 0 Then GoTo 10
    Set BadGuys(X).PicT = Form2.Alien1(BadGuys(X).indice)
    Set BadGuys(X).mask = Form2.Alien1M(BadGuys(X).indice)
    BadGuys(X).xsize = Form2.Alien1(BadGuys(X).indice).ScaleWidth
    BadGuys(X).ysize = Form2.Alien1(BadGuys(X).indice).ScaleHeight
    ...
    If BadGuys(X).activated = 1 And BadGuys(X).Exploding = 0 Then
      ...
      If mode = 3 Then
         ...
         If X = 0 And numsec < 1 Then
            ...
            BadGuys(X).X = Form1.PicMain.ScaleWidth '+ 10
         End If
         If X > 0 And numsec < 5 Then
            BadGuys(X).Y = Form1.PicMain.ScaleTop - 300
            BadGuys(X).X = Form1.PicMain.ScaleWidth '+ 10
         ...
         End If
         BadGuys(X).X = BadGuys(X).X - BadGuys(X).Velocity
         BadGuys(X).Y = BadGuys(X).Y + Sin(36 * Radians) + BadGuys(X).Velocity
      End If
      If mode = 4 Then
         BadGuys(X).Velocity = 20
         If X = 0 And numsec < 1 Then
            ...
            BadGuys(X).X = Form1.PicMain.ScaleWidth + 550
         End If
         If X > 0 And numsec < 5 Then
            BadGuys(X).Y = Form1.PicMain.ScaleHeight + 200
            BadGuys(X).X = Form1.PicMain.ScaleWidth + 550
